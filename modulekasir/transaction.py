...
class Transaction:
    ...
    def __init__(self) -> None:
        ...
        self._columns_name = ["No. Pembelian", "Nama Menu", "Harga", "Jumlah", "Total", "Tanggal"]
    ...
    def transaction(self):
        ...
        while True:
            ...
            print("="*15)
            menu, jumlah = input("Masukkan menu dan jumlah yang ingin dibeli (pisahkan dengan koma): ").split(",")
            if data_menu['Nama Menu'].str.contains(menu).any():
                harga = data_menu.loc[data_menu['Nama Menu'] == menu, 'Harga'].iloc[0]
                jumlah = self._data_menu.convert_types(jumlah, int)
                total_harga = harga * jumlah

                orders.loc[len(orders)] = [nomor_order, menu, harga, jumlah, total_harga, timestamp]

                # Update stok
                stok = data_menu.loc[data_menu['Nama Menu'] == menu, 'Stok'].iloc[0]
                stok -= int(jumlah)
